...
@section('title')
    Persetujuan Dokumen C300
@endsection

@section('content')

    <div class="container-xxl flex-grow-1 container-p-y">
        <h5 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Dosen /</span> Persetujuan Dokumen C300</h5>
        <!-- notification -->
        @include('template.notification')

        <!-- Bordered Table -->
        <div class="card">
            <h5 class="card-header">Data Persetujuan Dokumen C300</h5>

            <div class="card-body">
                <div class="row mb-2">
                    <div class="col-md-12">
                        <form class="form-inline" action="{{ url('/dosen/persetujuan-c300/search') }}" method="get"
                            autocomplete="off">
                            <div class="row">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @if ($rs_persetujuan_300->count() > 0)
                                @foreach ($rs_persetujuan_300 as $index => $kelompok)
                                    <tr>
                                        <td class="text-center">{{ $index + $rs_persetujuan_300->firstItem() }}.</td>
                                        <td>{{ $kelompok->nomor_kelompok }}</td>
                                        <td style="color: {{ $kelompok->status_dokumen_color }}">
                                            {{ $kelompok->file_status_c300 }}</td>
                                        <td>
                                            @if ($kelompok->file_path_c300 && $kelompok->file_name_c300)
                                                <a href="{{ asset($kelompok->file_path_c300 . '/' . $kelompok->file_name_c300) }}"
                                                    target="_blank">
                                                    Lihat File C300
                                                </a>
                                            @else
                ...
                <div class="row mt-3 justify-content-between">
                    <div class="col-auto mr-auto">
                        <p>Menampilkan {{ $rs_persetujuan_300->count() }} dari total
                            {{ $rs_persetujuan_300->total() }}
                            data.</p>
                    </div>
                    <div class="col-auto ">
                        {{ $rs_persetujuan_300->links() }}
                    </div>
                </div>
            </div>
        </div>
