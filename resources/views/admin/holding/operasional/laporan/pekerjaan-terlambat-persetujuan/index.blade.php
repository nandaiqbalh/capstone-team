...
@section('title')
    Rekapitulasi Pekerjaan Terlambat Persetujuan Seluruh Rumah Sakit
@endsection
...
@section('content')
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                ...
                <!-- Bordered Table -->
                <div class="card">
                    <h5 class="card-header">Rekapitulasi Pekerjaan Terlambat Persetujuan</h5>

                    <div class="card-body">    
                        <div class="row mb-2">
                            <div class="col-md-12">
                                <form class="form-inline" action="{{ url('/admin/holding-operasional/laporan/pekerjaan-terlambat-persetujuan/search') }}" method="get" autocomplete="off">
                                    <div class="row">
                                ...
                                <tbody>
                                    @if($rs_terlambat_persetujuan->count() > 0)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $terlambat_persetujuan->round_name }}</td>
                                            @foreach($rs_bulan as $key => $bulan)
                                            <td class="text-center">
                                                @if($terlambat_persetujuan->{$bulan} > 0)
                                                <a href="{{ url('/admin/holding-operasional/laporan/pekerjaan-terlambat-persetujuan/detail') }}/{{$terlambat_persetujuan->id}}/{{$key}}/{{$year}}">
                                                   {{ $terlambat_persetujuan->{$bulan} }}
                                                </a>
                                                @else
                                                {{ $terlambat_persetujuan->{$bulan} }}
                                                @endif
