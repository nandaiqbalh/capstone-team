...
@section('content')
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                <h5 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Laporan / </span>Rekapitulasi Pekerjaan</h5>
                <!-- notification -->
                @include("template.notification")

                <!-- Bordered Table -->
                <div class="card">
                    <h5 class="card-header">Rekapitulasi Pekerjaan</h5>

                    <div class="card-body">    
                        <div class="row mb-2">
                            <div class="col-md-12">
                                <form class="form-inline" action="{{ url('/admin/holding-operasional/laporan/pekerjaan/search') }}" method="get" autocomplete="off">
                                    <div class="row">
                                        <div class="col-md-9">
                                            <div class="row">
                                ...
                                <tbody>
                                    @if($rs_laporan_pekerjaan->count() > 0)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $laporan_pekerjaan->region_name }}</td>
                                            <td>{{ $laporan_pekerjaan->branch_name }}</td>
                                            <td class="text-center">{{ $laporan_pekerjaan->class }}</td>
                                            <td>{{ $laporan_pekerjaan->round_name }}</td>
                                            <td class="text-center">
                                                <a href="{{url('/admin/holding-operasional/laporan/pekerjaan/unduh-laporan')}}/{{ $laporan_pekerjaan->id }}" class="btn btn-outline-secondary btn-xs m-1">Unduh Laporan</a>
            ...
            <div class="modal fade" id="erRangkumanBCDModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="erRangkumanBCDModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-sm">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="erRangkumanBCDModalLabel">Rekapitulasi Seluruh Rumah Sakit</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="{{url('/admin/holding-operasional/laporan/pekerjaan/unduh-laporan-rekapitulasi-nilai')}}" method="post" id="form-rangkuman-abcd">
                        {{ csrf_field()}}
                            <div class="modal-body">
                                <div class="mb-3">
            ...
            <script type="text/javascript">
                ...
                function unduhLampiran(id) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{url('/admin/holding-operasional/laporan/pekerjaan/ajax-unduh-lampiran')}}",
                        data : {
                            _token : '{{csrf_token()}}',
                            id : id,
