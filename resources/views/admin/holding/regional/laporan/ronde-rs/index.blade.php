...
@section('title')
Rekapitulasi Ronde Seluruh Rumah Sakit Regional {{Auth::user()->region_id}}
@endsection

@section('content')
    
            <div class="container-xxl flex-grow-1 container-p-y">
                <h5 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Laporan / </span>Rekapitulasi Ronde Seluruh Rumah Sakit Regional {{Auth::user()->region_id}}</h5>
                <!-- notification -->
                @include("template.notification")

                <!-- Bordered Table -->
                <div class="card">
                    <h5 class="card-header">Rekapitulasi Ronde Seluruh Rumah Sakit Regional {{Auth::user()->region_id}}</h5>

                    <div class="card-body">    
                        <div class="row mb-2">
                            <div class="col-md-12">
                                <form class="form-inline" action="{{ url('/admin/holding-regional/laporan/ronde/search') }}" method="get" autocomplete="off">
                                    <div class="row">
                                        <div class="col-md-9">
                                            <div class="row">
                        ...
                        <div class="row justify-content-end mb-2">
                            <div class="col-auto ">
                            <a href="#" class="btn btn-primary btn-xs" data-bs-toggle="modal" data-bs-target="#erRangkumanABCDModal"><i class="bx bx-download"></i> Rekapitulasi Nilai</a>
                            </div>
                        </div>
                                ...
                                <tbody>
                                    @if($rs_laporan_ronde->count() > 0)
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">{{ $laporan_ronde->class }}</td>
                                            <td>{{ $laporan_ronde->round_name }}</td>
                                            <td class="text-center">
                                                <a href="{{url('/admin/holding-regional/laporan/ronde/unduh-laporan')}}/{{ $laporan_ronde->id }}" class="btn btn-outline-secondary btn-xs m-1">Unduh Laporan</a>
                                                ...
                                                <div class="modal-dialog modal-dialog-centered modal-sm">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            <button type="button" class="btn btn-primary btn-sm btn-unduh-lampiran" id="btn-unduh-lampiran-{{ $laporan_ronde->id }}" data-id="{{ $laporan_ronde->id }}">Unduh</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
            ...
            <!-- Modal -->
            <div class="modal fade" id="erRangkumanABCDModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="erRangkumanABCDModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-sm">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="erRangkumanABCDModalLabel">Rekapitulasi Nilai Seluruh Rumah Sakit</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="{{url('/admin/holding-regional/laporan/ronde/unduh-laporan-rekapitulasi-nilai')}}" method="post" id="form-rangkuman-abcd">
                        {{ csrf_field()}}
                        <input type="hidden" name="m_region_name" value="Regional {{Auth::user()->region_id}}" required>
                            <div class="modal-body">
                                <div class="mb-3">
            ...
            <script type="text/javascript">
                ...
                function unduhLampiran(id) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{url('/admin/holding-regional/laporan/ronde/ajax-unduh-lampiran')}}",
                        data : {
                            _token : '{{csrf_token()}}',
                            id : id,
