...
<link rel="stylesheet" href="{{asset('/vendor/libs/dropzone/dropzone593.min.css')}}" type="text/css" />
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                ...
                <!-- Bordered Table -->
                <div class="card">
                    <h5 class="card-header">Komponen Penilaian</h5>

                    <div class="card-body">    
                        <div class="row mb-2">
                            <div class="col-md-12">
                                <form class="form-inline" action="{{ url('/admin/validator/master/komponen-penilaian/search') }}" method="get" autocomplete="off">
                                    <div class="row">
                        ...
                        <div class="row justify-content-end mb-2">
                            <div class="col-auto ">
                                @if($role_id != '06')
                                <a href="{{ url('/admin/validator/master/komponen-penilaian/add') }}" class="btn btn-primary btn-xs float-right"><i class="bx bx-plus"></i> Tambah</a>
                                <a href="#" class="btn btn-warning btn-xs" data-bs-toggle="modal" data-bs-target="#uploadExcelModal"><i class="bx bxs-file-import"></i> Impor Data</a>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="table-responsive text-nowrap">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @if($rs_item_components->count() > 0)
                                        @foreach($rs_item_components as $index => $item_components)
                                        <tr>
                                            <td class="text-center">{{ $index + $rs_item_components->firstItem() }}</td>
                                            <td>{{ $item_components->items_name }}</td>
                                            <td>{{ $item_components->name }}</td>
                                            <td>{{ $item_components->parameter_true }}</td>
                                            <td class="text-center">
                                            @if($role_id != '06')
                                                <a href="{{ url('/admin/validator/master/komponen-penilaian/edit') }}/{{ $item_components->id }}" class="btn btn-outline-warning btn-xs float-right">Ubah</a>
                                                <a href="{{ url('/admin/validator/master/komponen-penilaian/delete_process') }}/{{ $item_components->id }}" class="btn btn-outline-danger btn-xs m-1 " onclick="return confirm('Apakah anda ingin menghapus?')">Hapus</a>
                        ...
                        <div class="row mt-3 justify-content-between">
                            <div class="col-auto ">
                                <p>Menampilkan {{ $rs_item_components->count() }} dari total {{ $rs_item_components->total() }} data.</p>
                            </div>
                            <div class="col-auto ">
                                {{ $rs_item_components->links() }}
                            </div>
                        </div>
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="uploadExcelModalLabel">Impor Data Komponen Penilaian</h5>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <form action="{{ url('/admin/validator/master/komponen-penilaian/import_data') }}" class="dropzone" id="my-dropzone" method="post" autocomplete="off" enctype="multipart/form-data">
                                            {{ csrf_field()}}
                                        </form>
