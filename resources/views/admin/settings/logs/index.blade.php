            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <ul class="nav nav-pills mb-3 " role="tablist">
                            ...
                            <li class="nav-item">
                                <a
                                href="{{url('admin/settings/logs/login')}}"
                                type="button"
                                class="nav-link"
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-2">
                            <div class="col-md-12">
                                <form class="form-inline" action="{{ url('/admin/settings/logs/search') }}" method="get" autocomplete="off">
                                    <div class="row">
                                        ...
                                        @if($rs_logs->count() > 0)
                                            ...
                                            <tr>
                                                <td class="text-center">
                                                    ...
                                                    <br>
                                                    <small>
                                                    <span class="text-lowercase">{{ strtolower($dtid->nicetime($log->timestamp)) }}</span>
                                                    </small>
                                                </td>
                                                <td class="text-center">{{ ucwords($log->env) }}</td>
                                                <td class="text-center">
                                                    @if(strtolower($log->type) == 'emergency' || strtolower($log->type) == 'alert' || strtolower($log->type) == 'critical' || strtolower($log->type) == 'error')
                                                    <span class="badge bg-danger">{{ ucwords(strtolower($log->type)) }}</span>
                                                    @elseif(strtolower($log->type) == 'warning' || strtolower($log->type) == 'notice')
                                                    <span class="badge bg-warning text-dark">{{ ucwords(strtolower($log->type)) }}</span>
                                                    @elseif(strtolower($log->type) == 'info')
                                                    <span class="badge bg-info text-dark">{{ ucwords(strtolower($log->type)) }}</span>
                                                    @else
                                                    <span class="badge bg-secondary">{{ ucwords(strtolower($log->type)) }}</span>
                                                    @endif
                                                </td>
                                                <td>{{ $log->message }}</td>
                                            </tr>
