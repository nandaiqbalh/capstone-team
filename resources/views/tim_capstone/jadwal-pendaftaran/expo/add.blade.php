...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h5 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Admin /</span> Jadwal Expo</h5>
        <!-- notification -->
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <small class="text-muted float-end">
                    <a href="{{ url('/admin/settings/contoh-halaman') }}" class="btn btn-secondary btn-xs float-right"><i
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" name="nomor_kelompok"
                                    value="{{ old('nomor_kelompok') }}" required>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                                ...
                                <select class="form-select select-2" name="id_mahasiswa" required>
                                    ...
                                    @foreach ($rs_mahasiswa as $mahasiswa)
                                        <option value="{{ $mahasiswa->user_id }}"
                                            @if (old('id_mahasiswa') == '{{ $mahasiswa->user_id }}') selected @endif>{{ $mahasiswa->user_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" name="judul_ta" value="{{ old('judul_ta') }}"
                                    required>
                            </div>
                        </div>
                    </div>
                                ...
                                <select class="form-select select-2" name="id_dosen1" required>
                                    ...
                                    @foreach ($rs_dosen as $dosen)
                                        <option value="{{ $dosen->user_id }}"
                                            @if (old('id_dosen1') == '{{ $dosen->user_id }}') selected @endif>{{ $dosen->user_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select select-2" name="id_dosen2" required>
                                    ...
                                    @foreach ($rs_dosen as $dosen)
                                        <option value="{{ $dosen->user_id }}"
                                            @if (old('id_dosen2') == '{{ $dosen->user_id }}') selected @endif>{{ $dosen->user_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
