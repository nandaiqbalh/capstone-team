...
class PekerjaanTerlambatPersetujuanModel extends BaseModel
{

    // get data
    public static function getDataSearch($query_all, $year) {
        // list ronde
        $rs_ronde = PekerjaanTerlambatPersetujuanModel::getListRonde($query_all);

        // perhitungan
        $rs_ronde->each(function($item, $key) use($year){
            
            $arr_terlambat_submit = PekerjaanTerlambatPersetujuanModel::getListTerlambatPersetujuanSemuaBulan($item->id, $year);
    ...
    public static function getDetail($round_id,$month, $year){
        return PekerjaanTerlambatPersetujuanModel::getDetailListTerlambatPersetujuanBy($round_id,$month, $year);
    }

    // get submit terlambat bulanan
    public static function getListTerlambatPersetujuanSemuaBulan($round_id, $year ) {
        $rs_bulan = parent::bulanIndo();

        $data = [];
        foreach ($rs_bulan as $key => $value) {
            $data[$value] = DB::table('branch_assignment as a')
    ...
    // public static function getDetailListTerlambatPersetujuanSemuaBulan($round_id, $year ) {
    //     $rs_bulan = parent::bulanIndo();

    //     $data = [];
    //     foreach ($rs_bulan as $key => $value) {

    //         $rs_branch = DB::table('branch_assignment as a')
    //             ->select('b.name')
    //             ->join('master_branch as b','a.branch_id','=','b.id')
    //             ->where('a.round_id', $round_id)
    //             ->whereNotNull('a.verifikator_2_approved_by_system')
    ...
    public static function getDetailListTerlambatPersetujuanBy($round_id,$month, $year ) {

        $data = DB::table('branch_assignment as a')
                ->select('b.name','b.class','b.region_name')
                ->join('master_branch as b','a.branch_id','=','b.id')
                ->where('a.round_id', $round_id)
                ->whereNotNull('a.verifikator_2_approved_by_system')
