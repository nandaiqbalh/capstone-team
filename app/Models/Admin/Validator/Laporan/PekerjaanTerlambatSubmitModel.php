...
class PekerjaanTerlambatSubmitModel extends BaseModel
{

    // get data
    public static function getDataSearch($query_all, $year) {
        // list ronde
        $rs_ronde = PekerjaanTerlambatSubmitModel::getListRonde($query_all);

        // perhitungan
        $rs_ronde->each(function($item, $key) use($year){
            
            $arr_terlambat_submit = PekerjaanTerlambatSubmitModel::getListTerlambatSubmitSemuaBulan($item->id, $year);
    ...
    public static function getDetail($round_id,$month, $year){
        return PekerjaanTerlambatSubmitModel::getDetailListTerlambatSubmitBy($round_id,$month, $year);
    }
