...
class PersetujuanC400Controller extends BaseController
{
    ...
    public function tolakPersetujuanC400Saya(Request $request, $id)
    {
        ...
        if (PersetujuanC400Model::updateKelompok($id, $params)) {
            ...
            if ($persetujuan_c400_updated->id == $id) {
                ...
                } else if ($persetujuan_c400_updated->file_status_c400_dosbing1 == "C400 Tidak Disetujui Dosbing 1" ) {

                        $paramsUpdated = [
                            'status_kelompok' => 'C400 Tidak Disetujui Dosbing 1',
                        ];
                        // Update status kelompok
                        PersetujuanC400Model::updateKelompok($id, $paramsUpdated);
                } else if ($persetujuan_c400_updated->file_status_c400_dosbing2 == "C400 Tidak Disetujui Dosbing 2" ) {
                        $paramsUpdated = [
                            'status_kelompok' => 'C400 Tidak Disetujui Dosbing 2',
                        ];
                        // Update status kelompok
                        PersetujuanC400Model::updateKelompok($id, $paramsUpdated);
                } else {
    ...
    public function search(Request $request)
    {
        ...
        if ($request->action == 'search') {
            // Get data with pagination
            $rs_persetujuan_400 = PersetujuanC400Model::getDataSearch($nama);

            // Check if result is null
            if (!$rs_persetujuan_400) {
                // Handle the case when no data is found
                $data = ['rs_persetujuan_400' => null, 'nama' => $nama];
            } else {
                ...
                $data = ['rs_persetujuan_400' => $rs_persetujuan_400, 'nama' => $nama];
            }
            ...
            return view('dosen.persetujuan-c400.index');
        }
    }
